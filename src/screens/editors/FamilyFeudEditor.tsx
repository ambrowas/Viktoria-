...
const FamilyFeudEditor: React.FC<FamilyFeudEditorProps> = ({ game, setGame }) => {
    ...
    const handleGenerate = async () => {
        ...
        setError('');
        try {
            const response = await generateFamilyFeudRound(aiTopic, lang);
            if (response.error) throw new Error(response.error);
            const newRound = response.data;
            if (!newRound) throw new Error("AI failed to generate round.");
            ...
            setGame({
                ...
                rounds: [
                    ...game.rounds,
                    {
                        ...
                        category: (newRound as any).category || aiTopic,
                        surveySize: (newRound as any).surveySize || 100
                    }
                ]
            });
            setAiTopic('');
