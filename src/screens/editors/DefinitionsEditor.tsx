...
const DefinitionsEditor: React.FC<DefinitionsEditorProps> = ({ game, setGame }) => {
  ...
  const handleGenerateAI = async () => {
    ...
    try {
      const response = await generateDefinitionsBulk(topic, 10, lang === "es" ? "es" : "en");
      ...
      setError(
        lang === "es"
          ? (err.message || "No se pudieron generar definiciones. Intenta de nuevo.")
          : (err.message || "Failed to generate definitions. Please try again.")
      );
    } finally {
