...
const ShowRunner: React.FC<ShowRunnerProps> = ({ show, games, onExit, initialState, isViewer = false }) => {
    ...
    const { isRemoteMode, updateSession, startSession, leaveSession, sessionId, sessionData, participants } = useSync();
    ...
    useEffect(() => {
        latestShowState.current = { step, currentRoundIndex, currentGameIndex, teamScores, countdown, participants };
    });

    const showBC = React.useRef<BroadcastChannel | null>(null);

    // PC: open channel
    useEffect(() => {
        if (isViewer) return;
        const bc = new BroadcastChannel('viktoria-show-state');
        showBC.current = bc;
        bc.onmessage = (event) => {
            if (event.data?.type === 'REQUEST_STATE') {
                const s = latestShowState.current;
                bc.postMessage({ type: 'STATE_UPDATE', ...s });
            }
        };
        return () => { bc.close(); showBC.current = null; };
    }, [isViewer]);

    // PC: broadcast on every state change (including participants)
    useEffect(() => {
        if (isViewer || !showBC.current) return;
        showBC.current.postMessage({ type: 'STATE_UPDATE', step, currentRoundIndex, currentGameIndex, teamScores, countdown, participants });
    }, [isViewer, step, currentRoundIndex, currentGameIndex, teamScores, countdown, participants]);

    // TV: receive and apply show state
    useEffect(() => {
        if (!isViewer) return;
        const bc = new BroadcastChannel('viktoria-show-state');
        bc.onmessage = (event) => {
            const { type, step: s, currentRoundIndex: r, currentGameIndex: g, teamScores: ts, countdown: cd, participants: pp } = event.data;
            if (type === 'STATE_UPDATE') {
                if (s !== undefined) setStep(s);
                if (r !== undefined) setCurrentRoundIndex(r);
                if (g !== undefined) setCurrentGameIndex(g);
                if (ts !== undefined) setTeamScores(ts);
                if (cd !== undefined) setCountdown(cd);
                if (pp !== undefined) setViewerParticipants(pp);
            }
        };
    ...
    useEffect(() => {
        ...
        const musicInLobby = show.settings.musicInLobby !== false; // default true

        if (step === "intro" || (step === "lobby" && musicInLobby) || step === "leaderboard" || step === "final_results") {
            if (isMusicPlaying) {
            ...
            // Fade out intro, then start countdown
            ...
            if (intro && !intro.paused) {
                fadeOutIntro(() => { stopAllSounds(); });
            } else {
                stopAllSounds();
            }
            countdownAudio?.pause();
            if (countdownAudio) countdownAudio.currentTime = 0;
        } else if (step === "lobby" && !musicInLobby) {
            // musicInLobby disabled: fade out
            if (intro && !intro.paused) fadeOutIntro(() => {});
            countdownAudio?.pause();
        } else {
            intro?.pause();
            countdownAudio?.pause();
                        ...
                        <div className="flex flex-col items-center gap-12">
                            <div className="relative w-48 h-48 flex items-center justify-center">
                                <svg className="absolute inset-0 w-full h-full -rotate-90">
                                    <circle cx="96" cy="96" r="88" fill="none" stroke="white" strokeOpacity="0.1" strokeWidth="8" />
                                    <motion.circle
                                ...
                                <div className="text-7xl font-black tabular-nums">{countdown}</div>
                            </div>
